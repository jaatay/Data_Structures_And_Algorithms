...
namespace FindIslands
{
	public class Graph
	{
		...
		/// <summary>
		/// method to determine number of islands in a graph
		/// </summary>
		/// <param name="nodeArray">2d array of nodes to search</param>
		/// <returns>decimal value rounded to nearest integer</returns>
		public decimal FindIslands(int[][] nodeArray)
		{
			int islandCounter = 0;
			...
			decimal finalCounter = islandCounter /= nodeArray.Length;
			finalCounter = Math.Ceiling(finalCounter);
			return finalCounter;
		}
		/// <summary>
